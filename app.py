...
# Name of the model actually loaded as `career_model.pkl` / used for
# predictions in get_predictions(). Must match a key in MODEL_METRICS
# exactly (check the printed MODEL_METRICS dict above at startup if
# unsure) — used to report the correct F1-score in /predict responses.
...
# ==========================================================
# Top TF-IDF Keywords for a Résumé
# ==========================================================
# Milestone 2 requirement: surface the highest-weighted TF-IDF terms for
# the résumé being analyzed, so the frontend can show which words drove
# the classification the most.

def get_top_tfidf_keywords(X_tfidf, top_n=10):
    """X_tfidf is the *un-selected* TF-IDF vector (output of
    vectorizer.transform), i.e. before feature_selector.transform() is
    applied — feature_selector can drop the human-readable term names,
    so we compute keywords from the raw TF-IDF matrix directly."""
    try:
        feature_names = vectorizer.get_feature_names_out()
        row = X_tfidf.toarray()[0]
        top_idx = np.argsort(row)[::-1][:top_n]
        keywords = [
            {"term": feature_names[i], "score": round(float(row[i]), 4)}
            for i in top_idx if row[i] > 0
        ]
        return keywords
    except Exception:
        print("⚠️  get_top_tfidf_keywords failed — full traceback below:")
        traceback.print_exc()
        return []
...
def predict():
    try:
        ...
        tfidf_keywords = get_top_tfidf_keywords(X_tfidf, top_n=10)

        X = feature_selector.transform(X_tfidf)

        recommendations, confidence = get_predictions(X, model, top_n=5)

        # Milestone 2: F1-score of the model actually used for this
        # prediction. MODEL_METRICS is keyed by model name (e.g.
        # "Logistic Regression", "Random Forest", "XGBoost") — PRIMARY_
        # MODEL_NAME below should match whichever entry corresponds to
        # `model` (the .pkl loaded at startup and used in get_predictions).
        # ⚠️ ASSUMPTION: adjust PRIMARY_MODEL_NAME if your career_model.pkl
        # is not Logistic Regression — it must exactly match a key in
        # model_metrics.json (or the fallback dict above) for this to
        # resolve correctly.
        f1_score = MODEL_METRICS.get(
            PRIMARY_MODEL_NAME,
            next(iter(MODEL_METRICS.values())) if MODEL_METRICS else None
        )
